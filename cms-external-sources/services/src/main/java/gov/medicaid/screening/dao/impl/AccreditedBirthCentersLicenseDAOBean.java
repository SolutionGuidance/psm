...
import com.topcoder.util.cache.jboss.JBossCacheAdapter;
...
public class AccreditedBirthCentersLicenseDAOBean extends BaseDAO implements AccreditedBirthCentersLicenseDAO {
    ...
    /**
     * It will be the cache for the data.
     */
    private Cache cache = new JBossCacheAdapter();
    ...
    private SearchResult<AccreditedBirthCenter> doSearch() throws URISyntaxException, IOException, ServiceException {
        ...
        List<AccreditedBirthCenter> accreditedBirthCenters;

        synchronized (cache) {
            accreditedBirthCenters = (List<AccreditedBirthCenter>) cache.get("accreditedBirthCenters");
        }

        if (accreditedBirthCenters == null) {
            accreditedBirthCenters = getAllResults();
            if (accreditedBirthCenters != null && accreditedBirthCenters.size() > 0) {
                synchronized (cache) {
                    cache.put("accreditedBirthCenters", accreditedBirthCenters);
                }
            }
        } else {
            getLog().log(Level.INFO, "cache hit on accreditedBirthCenters, skipped external site call.");
        }
