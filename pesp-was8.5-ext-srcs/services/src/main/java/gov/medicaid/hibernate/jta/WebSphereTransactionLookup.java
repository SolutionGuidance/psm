 ...
 */
package gov.medicaid.hibernate.jta;
...
/**
 * This class is responsible for looking up container managed transactions for
 * the hibernate provider and allowing synchronizations.
 * 
 * It is based on the original discussion from
 * https://forum.hibernate.org/viewtopic.php?f=1&t=992310 for websphere 6.1 but
 * is adapted for WAS 8.5
 * 
 * WAS does not allow direct access to the JTA unlike other J2EE servers.
 * 
 * Additional information can be found here:
 * http://pic.dhe.ibm.com/infocenter/wasinfo
 * /v8r5/topic/com.ibm.websphere.express.doc/ae/cjta_extjta.html
 * 
 * @author TCSASSEMBLER
 * @version 1.0
 */
public class WebSphereTransactionLookup extends AbstractJtaPlatform {
	 ...
	 * express.doc/ae/cjta_extjta.html
	 * 
	 * To obtain the UOWManager interface in a container-managed environment,
	 * use a JNDI lookup of java:comp/websphere/UOWManager. To obtain the
	 * UOWManager interface outside a container-managed environment, use the
	 * UOWManagerFactory class. This interface is available only in a server
	 * environment.
	 */
	...
	protected UserTransaction locateUserTransaction() {
		return (UserTransaction) jndiService()
				.locate(USER_TRANSACTION_LOCATION);
	}

	/**
	 * This adapter allows HIBERNATE to synchronize with WAS global
	 * transactions. Use only with CMT, where there is always a transaction in
	...
	public class TransactionManagerAdapter implements TransactionManager {
		...
		private UOWManager getUowManager() {
			try {
				...
				throw new IllegalStateException(
						"There is not UOWManager in the context, "
		...
		private UOWSynchronizationRegistry getUowSynchronizationRegistry() {
			try {
				InitialContext context = new InitialContext();
				return (UOWSynchronizationRegistry) context
						.lookup(SERVER_SYNCHRONIZATION_LOCATION);
			} catch (NamingException e) {
				throw new IllegalStateException(
						"There is not UOWManager in the context, "
								+ "make sure you are using this class inside a WAS container.");
		 ...
		 *             if the status could not be inspected
		 */
		 ...
		 * @throws SystemException
		 *             if the transaction could not be retrieved
		 */
		...
		/**
		 * Call is ignored. Since we assume that the transaction attribute is
		 * used to control isolation.
		 * 
		 * @param txn
		...
		/**
		 * Throws an exception when called because we cannot modify the global
		 * transaction.
		 */
		...
		/**
		 * This adapter allows inspection of some attributes of the global
		 * transaction.
		 * 
		 * @author TCSASSEMBLER
		 * @version 1.0
		...
		public class TransactionAdapter implements Transaction {
			...
			private TransactionAdapter() {
				extendedJTATransaction = (ExtendedJTATransaction) jndiService()
						.locate("java:comp/websphere/ExtendedJTATransaction");
			}
			...
			/**
			 * Throws an exception since we cannot roll back the global
			 * transaction.
			 * 
			 * @throws IllegalStateException
			 *             if called
