 ...
 */
@Stateless
@Local(PhysicalTherapyLicenseDAO.class)
@TransactionManagement(value = TransactionManagementType.CONTAINER)
@TransactionAttribute(value = TransactionAttributeType.REQUIRED)
public class PhysicalTherapyLicenseDAOBean extends BaseDAO implements PhysicalTherapyLicenseDAO {
    ...
    public SearchResult<License> searchByName(PhysicalTherapyLicenseSearchCriteria criteria) throws ServiceException {
        String signature = "PhysicalTherapyLicenseDataAccessImpl#searchByName";
        ...
        try {
            ...
            SearchResult<License> results = performSearch(NAME_CRITERIA, criteria.getLastName(), getSearchURL(),
    ...
    public SearchResult<License> searchByLicenseNumber(PhysicalTherapyLicenseSearchCriteria criteria)
        ...
        String signature = "PhysicalTherapyLicenseDataAccessImpl#searchByLicenseNumber";
        LogUtil.traceEntry(getLog(), signature, new String[]{"criteria"}, new Object[]{criteria});
        if (criteria == null) {
            throw new IllegalArgumentException("criteria must not be null.");
        }
    ...
    private License parseLicenseInfo(Elements tds) throws ParseException {
        ...
        User user = new User();
        String fullName = tds.get(1).html();
        ...
        Date issueDate = parseDate(tds.get(2).html(), DATE_FORMAT);
        if (issueDate != null) {
            license.setOriginalIssueDate(issueDate);
        }
        Date renewDate = parseDate(tds.get(3).html(), DATE_FORMAT);
        if (renewDate != null) {
            license.setRenewalDate(renewDate);
        }
        Date expireDate = parseDate(tds.get(4).html(), DATE_FORMAT);
        if (expireDate != null) {
            license.setExpireDate(expireDate);
        }
        Date ceDate = parseDate(tds.get(5).html(), DATE_FORMAT);
        if (ceDate != null) {
            license.setCeDate(ceDate);
        }

        String statusAndDate = tds.get(6).html();
        LicenseStatus status = new LicenseStatus();
        int statusEndIndex = statusAndDate.length();
        if (statusAndDate.indexOf("/") != -1) {
            statusEndIndex = statusAndDate.indexOf("/") - 2;
            String strDate = statusAndDate.substring(statusEndIndex, statusAndDate.length());
            license.setStatusDate(parseDate(strDate, DATE_FORMAT));
        }
        status.setName(statusAndDate.substring(0, statusEndIndex));
        license.setStatus(status);

        license.setDisciplineBdOrder(!"No".equals(tds.get(7).html()));
        license.setCorrectiveAction(!"No".equals(tds.get(8).html()));
        return license;
    }
}
