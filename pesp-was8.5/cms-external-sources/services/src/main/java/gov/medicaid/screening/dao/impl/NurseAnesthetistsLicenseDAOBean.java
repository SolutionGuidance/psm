...
public class NurseAnesthetistsLicenseDAOBean extends BaseDAO implements NurseAnesthetistsLicenseDAO {
    ...
    private ProviderProfile getProviderProfile(NurseAnesthetistsSearchCriteria criteria) throws ServiceException,
        ...
        HttpEntity entity = postForm(searchURL, client, search, new String[][]{{"__EVENTARGUMENT", ""},
            ...
            {"ctl00$PageContent$ucCredentialsControl$btnLookup", searchType},
            {"ctl00$PageContent$ucCredentialsControl$txtAANANumber", "" + criteria.getAanaNumber()},
            {"ctl00$PageContent$ucCredentialsControl$txtSSNLast4", "" + last4}}, true);
        ...
        if (criteria.isRecertification()) {
            ...
            entity = postForm(searchURL, client, search, new String[][]{{"__EVENTARGUMENT", ""}, {"__EVENTTARGET", ""},
                ...
                {"__VIEWSTATE", page.select("input[name=__VIEWSTATE]").first().val()},
                {"ctl00$PageContent$ucCredentialsControl$btnSwapDisplayMode", searchType}}, true);
            page = Jsoup.parse(EntityUtils.toString(entity));
        }
    ...
    private ProviderProfile parseProvider(Document page) throws ParsingException {
        ...
        String fullName = page.select("#ctl00_PageContent_ucCredentialsControl_lblMemberName").text();
        ...
        String issueDate = page.select("#ctl00_PageContent_ucCredentialsControl_lblInitialCertificationDate").text();
        if (Util.isNotBlank(issueDate)) {
            license.setOriginalIssueDate(parseDate(issueDate, DATE_FORMAT));
        }
        String renewalDate = page.select("#ctl00_PageContent_ucCredentialsControl_lblRecertBeginDate").text();
        if (Util.isNotBlank(renewalDate)) {
            license.setRenewalDate(parseDate(renewalDate, DATE_FORMAT));
        }

        String renewalEndDate = page.select("#ctl00_PageContent_ucCredentialsControl_lblRecertEndDate").text();
        if (Util.isNotBlank(renewalEndDate)) {
            license.setRenewalDate(parseDate(renewalEndDate, DATE_FORMAT));
        }
        String status = page.select("#ctl00_PageContent_ucCredentialsControl_lblRecertStatus").text();
        ...
        String licenseName = page.select("#ctl00_PageContent_ucCredentialsControl_lblAnesthesiaProgramName").text();
        ...
        String licenseNo = page.select("#ctl00_PageContent_ucCredentialsControl_lblAANANumber").text();
        license.setLicenseNumber(licenseNo);

        String address = page.select("#ctl00_PageContent_ucCredentialsControl_lblMemberResidence").text();
