...
The entire "psm-SQ-*" category is new, and has been added as a new
sheet in RTM.xlsx.  There are also some newly-created requirements.
See [[file:added-reqs.org][added-reqs.org]] for details about them -- the short story is: some
of them will likely disappear soon because they are redundant with
reqs that we discovered later when we un-hid some rows.  For now, they
are all listed at the end, after the "SQ" set, even though in RTM.xlsx
they have been added to appropriate sheets.
...
should just do it in RTM.xlsx, export all the sheets to CSV, and
then use the CSV files to get reqs in textual formats as needed.
