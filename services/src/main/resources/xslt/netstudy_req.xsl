...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="/">
		<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:impl="http://impl.services.screening.medicaid.gov/">
		   <soapenv:Header/>
		   <soapenv:Body>
		      <impl:getNETStudyResponse>
		         <arg0><xsl:value-of select="//ext:NETStudyVerificationRequest/ext:ProviderInformation/ext2:AgencyInformation/ext2:BackgroundStudyId/text()"></xsl:value-of></arg0>
		      </impl:getNETStudyResponse>
		   </soapenv:Body>
		</soapenv:Envelope>
    </xsl:template>
